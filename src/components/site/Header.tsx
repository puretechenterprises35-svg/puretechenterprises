...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-border/60 bg-background/85 backdrop-blur supports-[backdrop-filter]:bg-background/70">
      <div className="mx-auto grid max-w-7xl grid-cols-[auto_minmax(0,1fr)] items-center gap-4 px-4 py-3 sm:px-6 lg:px-8">
        <Link to="/" className="flex shrink-0 items-center gap-2">
          <img src={logoAsset.url} alt={`${site.name} logo`} className="h-10 w-auto shrink-0" width={200} height={100} />
          <span className="sr-only">{site.name}</span>
        </Link>

        <nav className="hidden items-center justify-self-end gap-1 xl:flex">
          {nav.map((n) => (
            <Link
              key={n.to}
              to={n.to}
